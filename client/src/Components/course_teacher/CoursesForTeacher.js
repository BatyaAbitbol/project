import { UseGetAll, UseGetAllById, UsePutOneById, UseGetOneByIdTeacher, UseGetOneById } from "../../Hooks/useGetAxios";
...
export default function CoursesForteacher(props) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const res = await UseGetOneById('teachers/course', id);
            if (res.status != 204)
                setData(res.data);
